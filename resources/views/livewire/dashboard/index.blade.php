<div class="space-y-5">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1 h-2 rounded-full bg-zinc-100 dark:bg-zinc-800">
                            <div class="h-2 rounded-full bg-{{ $color }}-500"
                                style="width: {{ ($statsByPriority[$label] / $maxPrior) * 100 }}%"></div>
                        </div>
                        <span
                            class="text-sm font-semibold text-zinc-900 dark:text-white w-8 text-right">{{ $statsByPriority[$label] }}</span>
                    </div>
                @endforeach
            </div>
        </flux:card>
        ...
        <flux:card
            ...
            <div class="space-y-2">
                @forelse ($perDepartemen as $dept)
                    <div
                        class="flex items-center justify-between py-1.5 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
                        <span class="text-sm text-zinc-700 dark:text-zinc-300">{{ $dept->name }}</span>
                        <span
                            class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $dept->tickets_count }}</span>
                    </div>
                @empty
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto">
                @forelse ($perKategori as $cat)
                    <div
                        ...
                        <span
                            class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $cat->tickets_count }}</span>
                    </div>
                @empty
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Belum ada data') }}</p>
                @endforelse
            ...
            <div class="space-y-1 max-h-80 overflow-y-auto">
                @forelse ($myTickets as $ticket)
                    <a href="{{ route('tickets.show', $ticket) }}" wire:navigate
                        class="flex items-center justify-between py-2 px-2 -mx-2 rounded hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-medium text-zinc-900 dark:text-white truncate">{{ $ticket->title }}
                            </p>
                            <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ $ticket->ticket_number }} &middot;
                                {{ $ticket->requester_name ?: $ticket->requester?->name }}</p>
                        </div>
    ...
    @if ($needsAction->isNotEmpty())
        <flux:card
            class="p-5 border-l-[3px] border-l-red-500 dark:border-l-red-400 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
            ...
            <div class="space-y-1">
                @foreach ($needsAction as $ticket)
                    <a href="{{ route('tickets.show', $ticket) }}" wire:navigate
                        ...
                        <div class="flex gap-1.5 shrink-0 ml-3">
                            <flux:badge color="{{ $ticket->priority === 'URGENT' ? 'red' : 'orange' }}" size="sm"
                                class="font-medium">
                                {{ $ticket->priority }}
                            </flux:badge>
                            <flux:badge color="{{ $ticket->status === 'WAITING_USER' ? 'orange' : 'green' }}"
                                size="sm" class="font-medium">
    ...
    {{-- Recent Tickets --}}
    <flux:card
        class="overflow-hidden border-l-[3px] border-l-zinc-400/50 dark:border-l-zinc-500/50 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
                ...
                <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800/50">
                    @forelse ($recentTickets as $ticket)
                        <tr onclick="window.location='{{ route('tickets.show', $ticket) }}'"
                            ...
                            <td class="px-5 py-3 text-sm font-medium text-zinc-900 dark:text-zinc-100">
                                {{ $ticket->title }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6"
                                class="px-5 py-10 text-center text-sm text-zinc-400 dark:text-zinc-500">
                                {{ __('Belum ada ticket') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
