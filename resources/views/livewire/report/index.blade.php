<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            <flux:button wire:click="resetFilters" variant="ghost" icon="x-mark" size="sm">{{ __('Reset') }}</flux:button>
            <flux:button wire:click="confirmExport" wire:loading.attr="disabled" icon="arrow-down-tray" variant="primary" size="sm">{{ __('Export XLSX') }}</flux:button>
        </div>
    </div>
    ...
    <flux:card class="p-5 border-l-[3px] border-l-violet-500 dark:border-l-violet-400 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <flux:field>
                ...
                <flux:input wire:model.live="startDate" type="date" class="dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
            </flux:field>
            <flux:field>
                <flux:label class="dark:text-zinc-300">{{ __('Sampai Tanggal') }}</flux:label>
                <flux:input wire:model.live="endDate" type="date" class="dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" />
            </flux:field>
            <flux:field>
            ...
            <flux:field>
                ...
                <flux:select wire:model.live="departmentId" class="dark:bg-zinc-800 dark:border-zinc-700 dark:text-white">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3">
        ...
        <flux:card class="p-4 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
            <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">{{ __('Waiting') }}</p>
            ...
            <table class="min-w-full divide-y divide-zinc-100 dark:divide-zinc-800">
                <thead>
                    <tr class="border-b border-zinc-100 dark:border-zinc-800">
                        ...
                        <th class="px-5 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">{{ __('Kategori') }}</th>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">{{ __('Assigned') }}</th>
                ...
                <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800/50">
                    @forelse ($tickets as $ticket)
                        <tr onclick="window.location='{{ route('tickets.show', $ticket) }}'" class="cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                            ...
                            <td class="px-5 py-3 text-sm font-medium text-zinc-900 dark:text-zinc-100 max-w-48 truncate">{{ $ticket->title }}</td>
                            <td class="px-5 py-3 text-sm text-zinc-600 dark:text-zinc-400">{{ $ticket->requester_name ?: $ticket->requester?->name }}</td>
                            <td class="px-5 py-3 text-sm text-zinc-600 dark:text-zinc-400">{{ $ticket->department?->name ?? '-' }}</td>
                            <td class="px-5 py-3 text-sm text-zinc-600 dark:text-zinc-400">{{ $ticket->category?->name ?? '-' }}</td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3">
                                <flux:badge color="{{ $ticket->status === 'OPEN' ? 'green' : ($ticket->status === 'CLOSED' ? 'zinc' : ($ticket->status === 'RESOLVED' ? 'purple' : 'yellow')) }}" size="sm" class="font-medium">
                                    {{ str_replace('_', ' ', $ticket->status) }}
                                </flux:badge>
                            </td>
                            <td class="px-5 py-3 text-sm text-zinc-600 dark:text-zinc-400">{{ $ticket->activeAssignment?->assignedTo?->name ?? '-' }}</td>
                            <td class="px-5 py-3 text-xs text-zinc-400 dark:text-zinc-500">{{ $ticket->created_at->format('d/m/Y H:i') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="px-5 py-10 text-center text-sm text-zinc-400 dark:text-zinc-500">{{ __('Tidak ada data') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
