<div x-data="{ previewUrl: '', previewName: '' }" class="space-y-5">
    ...
    <div class="flex items-start justify-between gap-4">
        <div class="min-w-0 flex-1" wire:poll.15s="checkNewComments">
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-xl font-semibold tracking-tight text-zinc-900 dark:text-white">{{ $ticket->ticket_number }}</h1>
        ...
        <div class="flex gap-2 shrink-0">
            @can('update', $ticket)
                <flux:button :href="route('tickets.edit', $ticket)" wire:navigate icon="pencil-square" variant="filled">{{ __('Edit') }}</flux:button>
            @endcan
            @can('close', $ticket)
                <flux:dropdown>
                    <flux:button icon="chevron-down" variant="primary">{{ __('Ubah Status') }}</flux:button>
                    <flux:menu>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 space-y-5">
            {{-- Detail Ticket --}}
            <flux:card class="p-5 border-l-[3px] border-l-zinc-400/50 dark:border-l-zinc-500/50 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
                <h2 class="text-sm font-semibold text-zinc-900 dark:text-white mb-4">{{ __('Detail Ticket') }}</h2>
                <p class="whitespace-pre-wrap text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed mb-4">{{ $ticket->description }}</p>
                <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                    <div class="flex items-center gap-2">
                        <span class="text-zinc-500 dark:text-zinc-400">{{ __('Requester') }}:</span>
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $ticket->requester_name ?: $ticket->requester?->name }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-zinc-500 dark:text-zinc-400">{{ __('Departemen') }}:</span>
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $ticket->department?->name ?? '-' }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-zinc-500 dark:text-zinc-400">{{ __('Kategori') }}:</span>
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $ticket->category?->name ?? '-' }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="text-zinc-500 dark:text-zinc-400">{{ __('Dibuat') }}:</span>
                        <span class="font-medium text-zinc-900 dark:text-white">{{ $ticket->created_at->format('d M Y H:i') }}</span>
                    </div>
                    @if ($currentAssignment)
                        <div class="flex items-center gap-2">
                            <span class="text-zinc-500 dark:text-zinc-400">{{ __('Assigned To') }}:</span>
                            <span class="font-medium text-zinc-900 dark:text-white">{{ $currentAssignment->assignedTo->name }}</span>
                        </div>
                    @endif
                    @if ($ticket->closed_at)
                        <div class="flex items-center gap-2">
                            <span class="text-zinc-500 dark:text-zinc-400">{{ __('Ditutup') }}:</span>
                            <span class="font-medium text-zinc-900 dark:text-white">{{ $ticket->closed_at->format('d M Y H:i') }}</span>
                        </div>
                    @endif
                </div>
            </flux:card>
                ...
                <flux:card class="p-5 border-l-[3px] border-l-blue-500 dark:border-l-blue-400 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
                    ...
                    <form wire:submit="assign" class="flex items-end gap-3">
                        <flux:field class="flex-1">
                            <flux:select wire:model="assignedUserId" class="dark:bg-zinc-800 dark:border-zinc-700 dark:text-white">
            ...
            @if ($attachments->isNotEmpty())
                <flux:card class="p-5 border-l-[3px] border-l-purple-500 dark:border-l-purple-400 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
                    <h2 class="text-sm font-semibold text-zinc-900 dark:text-white mb-4">{{ __('Lampiran') }} ({{ $attachments->count() }})</h2>
            ...
            @can('update', $ticket)
                <flux:card class="p-5 border-l-[3px] border-l-cyan-500 dark:border-l-cyan-400 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
                    ...
                    <form wire:submit="uploadAttachment" class="space-y-3">
                        ...
                        <div wire:loading wire:target="attachment" class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Mengupload...') }}</div>
                ...
                <div class="space-y-4">
                    @forelse ($comments as $comment)
                        <div class="border-b border-zinc-100 dark:border-zinc-800 pb-4 last:border-0">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-sm font-medium text-zinc-900 dark:text-zinc-100">{{ $comment->user->name }}</span>
                                <span class="text-xs text-zinc-500 dark:text-zinc-500">{{ $comment->created_at->diffForHumans() }}</span>
                                @if ($comment->is_internal)
                                    <flux:badge color="orange" size="sm" class="font-medium">{{ __('Internal') }}</flux:badge>
                                @endif
                            </div>
                    ...
                    @can('comment', $ticket)
                        <form wire:submit="addComment" class="space-y-3 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                            ...
                            @endcan
                            <flux:textarea wire:model="comment" rows="3" placeholder="{{ __('Tulis komentar...') }}" class="dark:bg-zinc-800 dark:border-zinc-700 dark:text-white dark:placeholder-zinc-500" />
        ...
        <div class="space-y-5">
            {{-- Riwayat --}}
            <flux:card class="p-5 border-l-[3px] border-l-amber-500 dark:border-l-amber-400 dark:bg-zinc-900 dark:border-zinc-700/50 shadow-card">
                ...
                <div class="space-y-3">
                    @forelse ($histories as $history)
                        <div class="text-sm border-l-2 border-zinc-200 dark:border-zinc-700 pl-3">
                            ...
                            <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ $history->performedBy?->name ?? '-' }}</div>
